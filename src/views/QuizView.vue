...
<h1>{{quiz.title}}</h1>
  <template v-if="!endScreen">
    <Question :qid="currQuestion" :question="quiz.questions[currQuestion]" @answered="addAnswer"></Question>
    <button v-if="currQuestion > 0" @click="currQuestion-=1; console.log(currQuestion)">Prev Question</button>
    <button v-if="currQuestion < quiz.questions.length - 1" @click="currQuestion+=1; console.log(currQuestion)">Next Question</button>
    <button v-if="Object.keys(answered).length == quiz.questions.length" @click="switchToEndScreen">End Quiz</button>
  </template>
  <template v-else>
    <div>
      <template v-for="question, ind in quiz.questions">
        ...
        <p :class="{correct: question.correctAnswer == answered[ind], incorrect: question.correctAnswer != answered[ind]}">answered: {{ question.answers[answered[ind]] }}</p>
      </template>
    </div>
  </template>
</template>
